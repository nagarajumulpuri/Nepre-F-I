...
def load_EnergyMatrix(cutoff):
    ...
    #List = sorted(List1) #.sort()
    f = open("./" + str(cutoff) + ".npy", 'rb')
    for amino1 in List:
        for amino2 in List:
            #aaDict[amino1][amino2] = np.load(f, allow_pickle=True)
...
def calculate_Energy(f,matrix,cutoff,fb_array):
    structure = f
    ...
    for i1 in range(tot_res):
        x_axis = record_Axis[i1][3]
        y_axis = record_Axis[i1][4]
        z_axis = record_Axis[i1][5]

        for i2 in range(tot_res):
            if(i1 == i2):
                continue
            else:
                if distanceMatrix[i1][i2] < cutoff :
                    ...
                    x = np.dot((center_i2 - center_i1), x_axis)
                    y = np.dot((center_i2 - center_i1), y_axis)
                    z = np.dot((center_i2 - center_i1), z_axis)
                    ...
                    fb_point = indexes
                    E += matrix[record_side_coord[i1][2]][record_side_coord[i2][2]][fb_point] / rho
                    #print ('length of pdb file :  ', len(record_N_coord))
    return E
...
if __name__ == "__main__":
    ...
    if(args.multi == True):
        ...
        for pdb_file in file_list:
            ...
            E.append(calculate_Energy(f,matrix,c,fb_array))
        if(args.output == True):
            with open("./latest_output.txt", "w") as fwout:
                fwout.write("Nepre Potential Energy ")
                fwout.write('\n')
                fwout.write("Using Cutoff: " + str(c))
                fwout.write('\n')
